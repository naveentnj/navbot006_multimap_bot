...
output_file_path = rospkg.RosPack().get_path('navbot006_multimap_bot')+"/saved_path/pose.csv"
rospy.loginfo('Starting a follow_waypoints')
waypoints = []
        ...
        def wait_for_start_journey():
            ...
            with open(output_file_path, 'r') as file:
                ...
                for row in reader:
                    ...
                    current_pose.pose.pose.position.x     =    2.44615#float(row[0])
                    current_pose.pose.pose.position.y     =    4.34028#float(row[1])
                    current_pose.pose.pose.position.z     =    5.38174 #float(row[2])
